...
def preprocess_data(df: pd.DataFrame, current_balance: float) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # 1. Сначала создаем отсортированную копию
    df_sorted = df.sort_values("Время открытия", ascending=False).reset_index(drop=True)

    # 2. Теперь считаем кумулятивную прибыль
    df_sorted["Кумулятивная прибыль"] = df_sorted["Прибыль числом"].cumsum()

    # 3. Рассчитываем баланс
    # Мы берем текущий баланс и вычитаем "будущую" (для этой точки) прибыль.
    # Чтобы последняя точка была равна current_balance, добавляем прибыль текущей сделки.
    df_sorted["Баланс"] = current_balance - df_sorted["Кумулятивная прибыль"] + df_sorted["Прибыль числом"]

    # 4. Снова сортируем по времени для правильного отображения графиков
